...
def cornersHeuristic(state, problem):
  ...
  if problem.isGoalState(state): #if the state is goal then return 0
    return 0

  cost = None
  count = 0
  for i in range(0, 4):
    if corner_state[i] == 0: #find how many corner unvisit
      count += 1
      position2 = corners[i]
      #compute the distance to the unvisited corner
      distance = util.manhattanDistance(position, position2)
  ...
  #if unvisit corner is 2, then chose the lowest cost way
  if count == 2:
    if len_y < len_x:
      cost += (len_y - 1)
    else:
      cost += (len_x - 1)
  elif count == 3: #if unvisit corner is 3, then add two way
    cost += (len_y - 1) + (len_x - 1)
  elif count == 4: #if unvisit corner is 4, then add two way and chose again the lowest cost way
    if len_y < len_x:
      cost += 2 * (len_y - 1) + (len_x - 1)
    else:
      cost += 2 * (len_x - 1) + (len_y - 1)
