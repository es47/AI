...
train_data = np.hstack((ad, st, sid, sc, at, ip, os, dt, pid, dv, cid, aid))
train_target = cl.astype('int')

# 讀入input資料

ad = pd.read_csv('input.csv', usecols=[0])
ad = np.asarray(ad)
st = pd.read_csv('input.csv', usecols=[1])
st = np.asarray(st)
sid = pd.read_csv('input.csv', usecols=[2])
sid = np.asarray(sid)
sc = pd.read_csv('input.csv', usecols=[3])
sc = np.asarray(sc)
at = pd.read_csv('input.csv', usecols=[4])
at = np.asarray(at)
ip = pd.read_csv('input.csv', usecols=[5])
ip = np.asarray(ip)
os = pd.read_csv('input.csv', usecols=[6])
os = np.asarray(os)
dt = pd.read_csv('input.csv', usecols=[7])
dt = np.asarray(dt)
pid = pd.read_csv('input.csv', usecols=[8])
pid = np.asarray(pid)
dv = pd.read_csv('input.csv', usecols=[9], dtype = {"dclkVerticals": str})
dv = np.asarray(dv)
cid = pd.read_csv('input.csv', usecols=[10])
cid = np.asarray(cid)
aid = pd.read_csv('input.csv', usecols=[11])
aid = np.asarray(aid)
...
test_data = np.hstack((ad, st, sid, sc, at, ip, os, dt, pid, dv, cid, aid))
...
data_clf = clf.fit(train_data, train_target)

# 預測
test_predicted = data_clf.predict(test_data)

output = {'click' : test_predicted}
...
"""# 績效
accuracy = metrics.accuracy_score(test_data, test_predicted)
print "accuracy : ", accuracy
precision = metrics.precision_score(test_data, test_predicted, average='macro')
print "precision : ", precision
recall = metrics.recall_score(test_data, test_predicted, average='macro')
print "recall : ", recall
